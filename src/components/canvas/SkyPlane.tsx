...
const SKY_FRAG = `
  ...
  // Simple 2D noise for subtle cloud texture
  float hash(vec2 p) {
  ...
  void main() {
    ...
    // Bright disc (only when sun is strong)
    if (uSunIntensity > 1.2) {
      float disc = smoothstep(0.022, 0.010, sunDist);
      color += uSunColor * disc * min(uSunIntensity * 0.4, 0.8);
    }

    // ── Subtle cloud noise on light stages ───────────────────────────────────
    float brightness = dot(color, vec3(0.299, 0.587, 0.114));
    if (brightness > 0.25) {
      float cloudT = fbm(vUv * 6.0 + vec2(uTime * 0.003, 0.0));
      cloudT = smoothstep(0.4, 0.7, cloudT) * 0.12 * brightness;
      color += vec3(cloudT * 0.9, cloudT * 0.88, cloudT * 0.85);
    }
...
export function SkyPlane() {
  ...
  // Smoothly track target values for buttery transitions
  ...
  useFrame(({ clock }) => {
    ...
    const s = 0.025 // smoothing factor
